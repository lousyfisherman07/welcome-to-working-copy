...
try:song=state.curSong.__get__(None)assertsong.ole()except.pnlnpmException.inQueue(print)ExceptioninsongsQueue FirstSong
	...
	while True:
		try:
			...
			assert song.openFile()
		except Exception:
printexceptioninsongsQueueongettingnextsongsys.exthook(*sys.exc_info())
	else:
			yield song
